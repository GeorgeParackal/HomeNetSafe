// ===== Config
  const API_URL = '/get_network_device_list'; // Flask endpoint

  // ===== Panel helper
  const panelButtons = document.querySelectorAll('[data-panel-button]');
  const stackPanes = document.querySelectorAll('.stack-pane');
  function setActivePanel(name){
    if (!name) return;
    stackPanes.forEach(pane => {
      const paneName = pane.dataset.panel || '';
      pane.classList.toggle('is-visible', paneName === name);
    });
    panelButtons.forEach(btn => {
      btn.classList.toggle('is-active', btn.dataset.panelButton === name);
    });
  }
  function switchToDevices(){ setActivePanel('devices'); }
  panelButtons.forEach(btn => {
    btn.addEventListener('click', () => setActivePanel(btn.dataset.panelButton));
  });
  switchToDevices();
  ...
  function updateLocalIpDisplay(ip){
    if (!localIpBanner || !localIpValue) return;
    if (ip){
      localIpValue.textContent = ip;
      localIpBanner.style.display = 'block';
    } else {
      localIpValue.textContent = '';
      localIpBanner.style.display = 'none';
    }
  }
  ...
  window.addEventListener('load', async ()=>{
    try{
      const { devices = [], localIp = null } = await runDeviceDiscovery();
      updateLocalIpDisplay(localIp);
      renderDevices(devices);
    }catch(err){
      console.error('Failed to run initial scan', err);
      updateLocalIpDisplay(null);
      renderDevices([]);
    }
  });
